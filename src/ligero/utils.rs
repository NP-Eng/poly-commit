...
pub(crate) fn calculate_t<F: PrimeField>(
    ...
    if !(rhs.is_normal()) {
        return Err(Error::InvalidParameters(format!("There are not enough bits in the field to support the requested security parameter. The field has {} bits, but the requested security parameter is {}.", field_bits, sec_param)));
    }
    let nom = rhs - 1.0;
