    ...
    for<'a, 'b> &'a P: Div<&'b P, Output = P>,
{
    ...
    type CommitmentState = MarlinCommitmentState<E::ScalarField, P>;
        ...
        Self::Commitment: 'a,
    {
        ...
        for (polynomial, state) in labeled_polynomials.into_iter().zip(states) {
            ...
            assert_eq!(degree_bound.is_some(), state.inner.shifted_rand.is_some());
            ...
            r += (challenge_j, &state.inner.rand);

            if let Some(degree_bound) = degree_bound {
                enforce_degree_bound = true;
                let shifted_rand = state.inner.shifted_rand.as_ref().unwrap();
